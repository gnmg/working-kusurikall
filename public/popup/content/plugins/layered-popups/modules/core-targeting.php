		...
		if (current_user_can('manage_options')) {
			...
			<div id="ulp-targets-userroles" class="ulp-targets-window-content-row'.(!$visitor_selected && empty($selected_roles) ? ' ulp-targets-disabled' : '').'">
				...
				<div class="ulp-targets-window-content-column ulp-targets-96pct">';
					...
					<a href="#" class="ulp-targets-userrole ulp-targets-input-item'.(!$visitor_selected && empty($selected_roles) ? ' ulp-targets-input-item-selected' : '').'" id="ulp-targets-userrole-all" onclick="return ulp_targets_userrole_selected(this);"><i class="far '.(!$visitor_selected && empty($selected_roles) ? ' fa-check-square' : 'fa-square').'"></i> '.__('All User Roles', 'ulp').'</a>
					<input type="hidden" class="ulp-userrole" id="ulp-userrole-visitor" name="ulp_userrole_visitor" value="'.($visitor_selected ? 'on' : 'off').'">
					<a href="#" class="ulp-targets-userrole ulp-targets-input-item'.($visitor_selected ? ' ulp-targets-input-item-selected' : '').'" id="ulp-targets-userrole-visitor" onclick="return ulp_targets_userrole_selected(this);"><i class="far '.($visitor_selected ? ' fa-check-square' : 'fa-square').'"></i> '.__('Non-registered Visitor', 'ulp').'</a>';
					...
					<a href="#" class="ulp-targets-userrole ulp-targets-input-item'.(in_array($key, $selected_roles) ? ' ulp-targets-input-item-selected' : '').'" id="ulp-targets-userrole-'.$key.'" onclick="return ulp_targets_userrole_selected(this);"><i class="far '.(in_array($key, $selected_roles) ? ' fa-check-square' : 'fa-square').'"></i> '.$value['name'].'</a>';
					}
					$html .= '
		...
		foreach ($taxonomies as $key => $taxonomy) {
			...
			<div class="ulp-targets-window-content-row ulp-targets-taxonomies'.(!$selected ? ' ulp-targets-disabled' : '').'">
				...
				<div class="ulp-targets-window-content-column ulp-targets-96pct">
					...
					<a href="#" class="ulp-targets-taxonomy-'.$key.' ulp-targets-input-item'.(!$selected ? ' ulp-targets-input-item-selected' : '').'" id="ulp-targets-taxonomy-'.$key.'-all" onclick="return ulp_targets_taxonomy_selected(this, \''.$key.'\');"><i class="far '.(!$selected ? ' fa-check-square' : 'fa-square').'"></i> '.$taxonomy->labels->all_items.'</a>';
			foreach ($terms as $term) {
				...
				$html .= '
					...
					<a href="#" class="ulp-targets-taxonomy-'.$key.' ulp-targets-input-item'.(in_array($term->slug, $selected_terms) ? ' ulp-targets-input-item-selected' : '').'" id="ulp-targets-taxonomy-'.$key.'-'.$term_id.'" onclick="return ulp_targets_taxonomy_selected(this, \''.$key.'\');"><i class="far '.(in_array($term->slug, $selected_terms) ? 'fa-check-square' : 'fa-square').'"></i> '.esc_html($term->name).'</a>';
			}
			$html .= '
				</div>';
			if (!array_key_exists($_event, $this->inline_events)) {
				...
				<div class="ulp-targets-window-content-column ulp-targets-96pct ulp-targets-taxonomy-archive-enable">
					...
					<a href="#" id="ulp-targets-taxonomy-'.$key.'-archive-enable" onclick="return ulp_targets_taxonomy_archive_enbaled(\''.$key.'\');"><i class="far '.($archive_enable == 'on' ? 'fa-check-square' : 'fa-square').'"></i> '.sprintf(__('Enable popup for selected "%s" archive pages', 'ulp'), $taxonomy->label).'</a>
				</div>';
			}
			$html .= '
			</div>';
		...
		$html .= '
			...
			<div class="ulp-targets-window-content-row ulp-targets-posts">
				...
				<div class="ulp-targets-window-content-column ulp-targets-96pct">
					...
					<a href="#" class="ulp-targets-input-item'.($target['posts_all'] ? ' ulp-targets-input-item-selected' : '').'" id="ulp-targets-post-all" onclick="return ulp_targets_post_all_selected();"><i class="far '.($target['posts_all'] ? ' fa-check-square' : 'fa-square').'"></i> '.sprintf(__('All %s', 'ulp'), strtolower($post_type->labels->name)).'</a>
	...
	function get_posts_html($_target, $_offset = 0) {
		...
		if ($_offset == 0) {
			...
			<div class="ulp-targets-posts-item">
				...
				<a href="#" id="ulp-targets-post-'.$post->ID.'" onclick="return ulp_targets_post_selected('.$post->ID.');"><i class="far fa-check-square"></i> '.(empty($post->post_title) ? __('No title', 'ulp') : esc_html($post->post_title)).' (ID: '.$post->ID.', Status: '.ucfirst($post->post_status).')</a>
			</div>';
				}
		...
		foreach ($query->posts as $post) {
			...
			<div class="ulp-targets-posts-item">
				...
				<a href="#" id="ulp-targets-post-'.$post->ID.'" onclick="return ulp_targets_post_selected('.$post->ID.');"><i class="far fa-square"></i> '.(empty($post->post_title) ? __('No title', 'ulp') : esc_html($post->post_title)).' (ID: '.$post->ID.', Status: '.ucfirst($post->post_status).')</a>
			</div>';
		}
		if ($query->found_posts > 0) $posts_found = true;
		if (!$posts_found) {
		...
		if (current_user_can('manage_options')) {
			...
			if ($target['post_type'] != 'sitewide' && $target['post_type'] != 'homepage') {
				...
				foreach ($taxonomies as $key => $taxonomy) {
					...
					if (array_key_exists('ulp_taxonomy_'.$key.'_archive_enable', $_REQUEST)) $target['taxonomies']['archive_enable_'.$key] = $_REQUEST['ulp_taxonomy_'.$key.'_archive_enable'];
					else $target['taxonomies']['archive_enable_'.$key] = 'off';
				}
				$target['posts_all'] = (array_key_exists('ulp_post_all', $_REQUEST) && $_REQUEST['ulp_post_all'] == 'on') ? true : false;
				if (!$target['posts_all']) {
